...
ENV container=oci \
    MANPATH=/opt/rh/go-toolset-1.11/root/usr/share/man: \
    X_SCLS=go-toolset-1.11  \
    LD_LIBRARY_PATH=/opt/rh/go-toolset-1.11/root/usr/lib64 \
    PATH=/opt/rh/go-toolset-1.11/root/usr/bin:/opt/rh/go-toolset-1.11/root/usr/sbin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
    GOPATH=/go \
    PKG_CONFIG_PATH=/opt/rh/go-toolset-1.11/root/usr/lib64/pkgconfig
...
RUN yum install -y --setopt=skip_missing_names_on_install=False \
    ...
    go-toolset-1.11 goversioninfo openssl openssl-devel systemd-devel
RUN yum clean all
